...
enum OperatingSystemDescription {
  ...
  static {
    for (OperatingSystemDescription operatingSystemDescription :
        OperatingSystemDescription.values()) {
  ...
  /**
   * The Sauce REST API returns the server operating system name (eg. Windows 2003) rather than the
   * public name (eg. Windows XP), so if we've defined a mapping for the description, we use that
   * here.
   *
   * @param osName
   * @return
